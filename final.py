...
def get_frames(color, duration):
    cap = cv2.VideoCapture(0)
    frames = []
    total_frames = 60 * duration
    count = 0
...
def pupilDiameter(color, duration):
    ...
    frames = get_frames(color, duration)
    ...
    for frame in frames:
        ...
        if (len(eyes)>0):
            ...
            cv2.putText(frame,a,(10,30), font, 1,(0,0,255),2,cv2.LINE_AA)
                ...
                try:
                    for i in circles[0,:]:
                        ...
                        if(i[2]>0 and i[2]<155):
                            ...
                            cv2.putText(frame,"Pupil Pos:",(450,30), font, 1,(0,0,255),2,cv2.LINE_AA)
                            cv2.putText(frame,"X "+str(int(i[0]))+" Y "+str(int(i[1])),(430,60), font, 1,(0,0,255),2,cv2.LINE_AA)
                            d = (int(i[2])*2.0)
                            dmm = 1/(25.4/d)
                            print(dmm)
                            if(color == 1):
                            ...
                            cv2.putText(frame,str('{0:.2f}'.format(dmm))+"mm",(10,60), font, 1,(0,0,255),2,cv2.LINE_AA)
            ...
            if (blink==False):
                blink=True
                if blink==True:
                        cv2.putText(frame,"Blink",(10,90), font, 1,(0,0,255),2,cv2.LINE_AA)
            a="Eye Close" 
            cv2.putText(frame,a,(10,30), font, 1,(0,0,255),2,cv2.LINE_AA)
